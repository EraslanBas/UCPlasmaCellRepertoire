---
title: "Per donor transcriptome analysis without batch correction"
output:
  html_document:
    df_print: paged
---
...
# 
# 
#  for(elem in c("UC3" ,"UC10", "UC15", "UC16", "UC17", "UC18", "UC20", "Healthy1", "Healthy2", "Healthy3",  "Healthy4","UC4","UC9","UC14","UC19")){
...
#    }
#  
#    saveRDS(donorObjects, paste0(dataDir, "/RDSFiles/donorObjects.rds"))
#  
#names(donorObjects) <- c("UC3" ,"UC10", "UC15", "UC16", "UC17", "UC18", "UC20", "Healthy1", "Healthy2", "Healthy3",  "Healthy4","UC4","UC9","UC14","UC19")
...
```{r echo=FALSE, error=FALSE, message=FALSE, warning=FALSE, echo=FALSE, fig.width=10, fig.height=8}
  ...
  allObjMarkers <- readRDS(paste0(dataDir, "/RDSFiles/allObjMarkersPerDonor.rds"))
  #allObjMarkers <- allObjMarkers[names(donorObjects)]
  #allObjMarkers <- list()
  for(i in 1:length(donorObjects)){
    ...
    print(DotPlot(donObj, assay="RNA", features = unlist(list("plasma cells"=c("SDC1", "CD27", "CD38"),
     ...
     print(DimPlot(donObj, reduction = "umap", group.by = "sampleName", pt.size = 0.05)+
    ...
    if(length(unique(donObj$seurat_clusters)) > 1){
        ...
        for(myClustI in unique(donObj$seurat_clusters)){
         ...
         #           dot.scale = 3,  col.min = 0, col.max = 6) +
         #           RotatedAxis()+theme_bw()+theme(axis.text.x = element_text(angle = 90, hjust = 1))+ggtitle(paste0(unique(donObj$orig.ident))))

        #getDonorClusterGOTerms(donObj, obj.markers,fileAppendStr="PerDonorNC")

    #  donObj$lane <- as.factor(donObj$lane)
    #  print(DimPlot(donObj, reduction = "umap", group.by = "lane", pt.size = 0.05)+
    ...
    #  print(DimPlot(donObj, reduction = "umap", group.by = "replicates", pt.size = 0.05)+
    ...
    #  print(DimPlot(donObj, reduction = "umap", group.by = "stim", pt.size = 0.05)+
    ...
    #  print(DimPlot(donObj, reduction = "umap", group.by = "position", pt.size = 0.05)+
    ...
    # donObj <- CellCycleScoring(donObj, s.features = s.genes, g2m.features = g2m.genes, set.ident = TRUE)
    # 
    # print(DimPlot(donObj, reduction = "umap", split.by = "stim", pt.size = 0.05)+
    #          ggtitle(paste0(unique(donObj$orig.ident))," sample attribute"))
     # donObj$heavyChain <- as.factor(donObj$heavyChain)
     # print(DimPlot(donObj, reduction = "umap", group.by = "heavyChain", pt.size = 0.05)+
     ...
     # print(DimPlot(donObj, reduction = "umap", group.by = "lightChain", pt.size = 0.05)+
  ...
  #names(allObjMarkers) <- names(donorObjects)
  #saveRDS( allObjMarkers, paste0(dataDir, "/RDSFiles/allObjMarkersPerDonor.rds"))
